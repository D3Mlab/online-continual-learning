...
class SupContrastPure(ContinualLearner):
    def __init__(self, model, opt, params):
        ...
        params.mem_size = int(params.mem_size*0.1)
                ...
                if batch_aux_y.size(0) > 0:
                    for j in range(self.mem_iters):
                        ...
                        if candidate_x.size(0) > 0:
                            ...
                            # combined_batch = torch.cat((match_x, batch_x, candidate_x, batch_aux_x))
                            # combined_labels = torch.cat((match_y, batch_y, candidate_y, batch_aux_y))
                            ...
                            anchor_x = torch.cat((candidate_x, batch_x))
                            anchor_y = torch.cat((candidate_y, batch_y))

                            view1_x = torch.cat((match_x, batch_aux_x))
                            view2_x = self.transform(torch.cat((candidate_x, batch_x)))

                            features = torch.cat([self.model.forward(anchor_x).unsqueeze(1),
                                                  self.model.forward(view1_x).unsqueeze(1),
                                                  self.model.forward(view2_x).unsqueeze(1)
                                                  ], dim=1)

                            loss = self.criterion(features, anchor_y)
